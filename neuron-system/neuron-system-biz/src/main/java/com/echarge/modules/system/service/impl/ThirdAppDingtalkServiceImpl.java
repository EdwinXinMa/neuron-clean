...
import com.echarge.common.constant.enums.MessageTypeEnum;
...
import com.echarge.modules.system.model.ThirdLoginModel;
import com.echarge.modules.system.service.*;
import com.echarge.modules.system.vo.thirdapp.JdtDepartmentTreeVo;
import com.echarge.modules.system.vo.thirdapp.SyncInfoVo;
...
public class ThirdAppDingtalkServiceImpl implements IThirdAppService {
    ...
    @Autowired
    NeuronBaseConfig neuronBaseConfig;
    @Autowired
...
//    }
    ...
    public Response<String> sendActionCardMessage(SysAnnouncement announcement, String ddMobileUrl, boolean verifyConfig) {
        ...
        String baseUrl = null;
        //优先通过请求获取basepath，获取不到读取 neuron.domainUrl.pc
        try {
            baseUrl = RestUtil.getBaseUrl();
        } catch (Exception e) {
            log.warn(e.getMessage());
            baseUrl =  neuronBaseConfig.getDomainUrl().getPc();
            //e.printStackTrace();
        }
    ...
    public SysUser oauth2Login(String authCode,Integer tenantId) {
        ...
        if (userAccessToken == null) {
            ...
            throw new NeuronBootException("请查看应用key和应用秘钥是否正确，组织ID是否匹配");
        }
        ...
        if (contactUser == null) {
            ...
            throw new NeuronBootException("获取钉钉用户信息失败");
        }
    ...
    public SyncInfoVo syncThirdAppDepartmentUserToLocal() {
        ...
        if(departments.isEmpty()){
            throw new NeuronBootBizTipException("请查看配置参数和白名单是否配置！");
        }
        String username = JwtUtil.getUserNameByToken(SpringContextUtils.getHttpServletRequest());
        List<JdtDepartmentTreeVo> departmentTreeList = JdtDepartmentTreeVo.listToTree(departments);
        int tenantId = oConvertUtils.getInt(TenantContext.getTenant(), 0);
    ...
    public void tenantIzExist(Integer tenantId){
        if(MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL){
            ...
            if(ObjectUtil.isEmpty(count) || 0 == count){
                throw new NeuronBootException("租户ID:" + tenantId + "无效，平台中不存在！");
            }
        }
    }
    ...
    public SysUser oauthDingDingLogin(String authCode, Integer tenantId) {
        ...
        if (userInfoResponse.isSuccess()) {
            ...
            if (thirdAccount != null) {
                ...
                throw new NeuronBootException("该用户没有同步，请先同步！");
            }
        }
        return null;
    }
