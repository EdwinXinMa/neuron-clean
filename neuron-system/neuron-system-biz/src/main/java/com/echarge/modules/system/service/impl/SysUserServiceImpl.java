...
import com.echarge.common.api.dto.message.MessageDTO;
...
import com.echarge.common.constant.FillRuleConstant;
import com.echarge.common.constant.SymbolConstant;
import com.echarge.common.constant.PasswordConstant;
import com.echarge.common.constant.enums.*;
import com.echarge.common.desensitization.annotation.SensitiveEncode;
import com.echarge.common.exception.NeuronBootBizTipException;
...
import com.echarge.common.util.*;
import com.echarge.common.util.encryption.AesEncryptUtil;
...
import com.echarge.modules.system.entity.*;
import com.echarge.modules.system.mapper.*;
import com.echarge.modules.system.model.SysLoginModel;
import com.echarge.modules.system.model.SysUserSysDepPostModel;
import com.echarge.modules.system.model.SysUserSysDepartModel;
import com.echarge.modules.system.service.*;
import com.echarge.modules.system.util.ImportSysUserCache;
import com.echarge.modules.system.vo.*;
import com.echarge.modules.system.vo.lowapp.AppExportUserVo;
import com.echarge.modules.system.vo.lowapp.DepartAndUserInfo;
import com.echarge.modules.system.vo.lowapp.DepartInfo;
import com.echarge.modules.system.vo.lowapp.UpdateDepartInfo;
...
//		this.save(user);  //保存角色的时候已经添加过一次了
    ...
    public void userQuit(String username) {
        SysUser sysUser = userMapper.getUserByName(username);
        if(null == sysUser){
            throw new NeuronBootException("离职失败，该用户已不存在");
        }
		// 代码逻辑说明: [QQYUN-3951]租户用户离职重构------------
		int tenantId = oConvertUtils.getInt(TenantContext.getTenant(), 0);
		//更新用户租户表的状态为离职状态
		if(tenantId==0){
			throw new NeuronBootException("离职失败，租户不存在");
		}
...
//		}
	...
	public void checkUserAdminRejectDel(String userIds) {
		...
		//大于0说明存在管理员用户，不允许删除
		if(adminRoleCount>0){
			throw new NeuronBootException("admin用户，不允许删除！");
		}
	}
	...
	public void changePhone(JSONObject json, String username) {
		...
		if(oConvertUtils.isEmpty(phone)){
			throw new NeuronBootException("请填写原手机号！");
		}
		if(oConvertUtils.isEmpty(smscode)){
			throw new NeuronBootException("请填写验证码！");
		}
		//step1 验证原手机号是否和当前用户匹配
		SysUser sysUser = userMapper.getUserByNameAndPhone(phone,username);
		if (null == sysUser){
			throw new NeuronBootException("原手机号不匹配，无法修改密码！");
		}
		...
		if(CommonConstant.VERIFY_ORIGINAL_PHONE.equals(type)){
			...
			if(newPhone.equals(phone)){
				throw new NeuronBootException("新手机号与原手机号一致，无法修改！");
			}
	...
	public void verifyPhone(String phone, String smsCode){
		...
		if(null == phoneCode){
			throw new NeuronBootException("验证码失效，请重新发送验证码！");
		}
		if(!smsCode.equals(phoneCode.toString())) {
			throw new NeuronBootException("短信验证码不匹配！");
		}
		//验证完成之后清空手机验证码
	...
	public void sendChangePhoneSms(JSONObject jsonObject, String username, String ipAddress) {
		...
		if(oConvertUtils.isEmpty(phone)){
			throw new NeuronBootException("请填写手机号！");
		}
		...
		if(CommonConstant.VERIFY_ORIGINAL_PHONE.equals(type)){
			...
			SysUser sysUser = userMapper.getUserByNameAndPhone(phone, username);
			if(null == sysUser){
				throw new NeuronBootException("旧手机号不匹配，无法修改手机号！");
			}
		}else if(CommonConstant.UPDATE_PHONE.equals(type)){
			//step3 新手机号需要验证手机号码是否已注册过
			SysUser userByPhone = userMapper.getUserByPhone(phone);
			if(null != userByPhone){
				throw new NeuronBootException("手机号已被注册，请尝试其他手机号！");
			}
		}
		//step4 发送短信验证码
	...
	public void sendLogOffPhoneSms(JSONObject jsonObject, String username, String ipAddress) {
		...
		if (null == userByNameAndPhone) {
			throw new NeuronBootException("当前用户手机号不匹配，无法修改！");
		}
	...
	public void userLogOff(JSONObject jsonObject, String username) {
		...
		SysUser userByNameAndPhone = userMapper.getUserByNameAndPhone(phone, username);
		if (null == userByNameAndPhone) {
			throw new NeuronBootException("当前用户手机号不匹配，无法注销！");
		}
		String code = CommonConstant.LOG_OFF_PHONE_REDIS_KEY_PRE + phone;
		Object redisSmdCode = redisUtil.get(code);
		if (null == redisSmdCode) {
			throw new NeuronBootException("验证码失效，无法注销！");
		}
		if (!redisSmdCode.toString().equals(smsCode)) {
			throw new NeuronBootException("验证码不匹配，无法注销！");
		}
	...
	private void sendPhoneSms(String phone, String clientIp,String redisKey) {
		...
		if (object != null) {
			throw new NeuronBootException("验证码10分钟内，仍然有效！");
		}

		//增加 check防止恶意刷短信接口
		if(!DySmsLimit.canSendSms(clientIp)){
			log.warn("--------[警告] IP地址:{}, 短信接口请求太多-------", clientIp);
			throw new NeuronBootException("短信接口请求太多，请稍后再试！", CommonConstant.PHONE_SMS_FAIL_CODE);
		}
    ...
    public void updatePasswordNotBindPhone(String oldPassword, String password, String username) {
        ...
        if(!sysUser.getUsername().equals(username)){
            throw new NeuronBootBizTipException("只允许修改自己的密码！");
        }
        //step2 用户不存在禁止修改密码
        SysUser user = this.getUserByName(username);
        if(null == user){
            throw new NeuronBootBizTipException("用户不存在，无法修改密码！");
        }
        //setp3 如果手机号存在需要用手机号修改密码的方式
        if(oConvertUtils.isNotEmpty(user.getPhone())){
            throw new NeuronBootBizTipException("手机号不为空，请根据手机号进行修改密码操作！");
        }
        //step4 判断旧密码是否正确
        String passwordEncode = PasswordUtil.encrypt(username, oldPassword, user.getSalt());
        if (!user.getPassword().equals(passwordEncode)) {
            throw new NeuronBootBizTipException("旧密码输入错误!");
        }
        if (oConvertUtils.isEmpty(password)) {
            throw new NeuronBootBizTipException("新密码不允许为空!");
        }
        //step5 修改密码
	...
	public Result loginGetUserDeparts(JSONObject jsonObject) {
		...
		if("phone".equalsIgnoreCase(loginType)){
			...
			if(!"APP".equalsIgnoreCase(source)){
				...
				String keyPrefix = Md5Util.md5Encode(sysLoginModel.getCheckKey() + neuronBaseConfig.getSignatureSecret(), "utf-8");
	...
	public void resetToSysPassword(String usernames) {
		...
		if(hasAdminIntersection(usernames)){
			throw new NeuronBootException("所选用户中包含管理员，管理员账号不允许重置密码！！");
		}
