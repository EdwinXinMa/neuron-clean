...
import com.echarge.common.constant.enums.DySmsEnum;
...
import com.echarge.common.util.encryption.EncryptedString;
import com.echarge.config.NeuronBaseConfig;
import com.echarge.config.shiro.IgnoreAuth;
import com.echarge.modules.base.service.BaseCommonService;
import com.echarge.modules.system.entity.SysDepart;
import com.echarge.modules.system.entity.SysRoleIndex;
...
import com.echarge.modules.system.util.RandImageUtil;
...
public class LoginController {
	...
	private void handleSingleSignOn(String username, String newToken, String clientType) {
		// 检查是否允许并发登录
		if (neuronBaseConfig.getFirewall() == null || neuronBaseConfig.getFirewall().getIsConcurrent()==null || Boolean.TRUE.equals(neuronBaseConfig.getFirewall().getIsConcurrent())) {
	...
	public Result<String> randomImage(HttpServletResponse response,@PathVariable("key") String key){
		...
		try {
			...
			String keyPrefix = Md5Util.md5Encode(key + neuronBaseConfig.getSignatureSecret(), "utf-8");
...
//	}
	...
	public Result<?> smsCheckCaptcha(@RequestBody SysLoginModel sysLoginModel, HttpServletRequest request){
		...
		String lowerCaseCaptcha = captcha.toLowerCase();
		String realKey = Md5Util.md5Encode(lowerCaseCaptcha+checkKey+neuronBaseConfig.getSignatureSecret(), "utf-8");
		Object checkCode = redisUtil.get(realKey);
		if(checkCode==null || !checkCode.equals(lowerCaseCaptcha)) {
			return Result.error("验证码错误");
		}
    ...
    private String validateCaptcha(SysLoginModel sysLoginModel, Result<JSONObject> result) {
		// 判断是否启用登录验证码校验
		if (neuronBaseConfig.getFirewall() != null && Boolean.FALSE.equals(neuronBaseConfig.getFirewall().getEnableLoginCaptcha())) {
