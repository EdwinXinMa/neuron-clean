...
import com.echarge.common.api.dto.message.BusMessageDTO;
...
import com.echarge.common.constant.enums.SysAnnmentTypeEnum;
import com.echarge.common.util.oConvertUtils;
import com.echarge.modules.aop.TenantLog;
import com.echarge.modules.system.entity.*;
import com.echarge.modules.system.mapper.*;
import com.echarge.modules.system.service.ISysTenantPackService;
import com.echarge.modules.system.service.ISysTenantService;
import com.echarge.modules.system.service.ISysUserService;
import com.echarge.modules.system.vo.tenant.*;
...
public class SysTenantServiceImpl extends ServiceImpl<SysTenantMapper, SysTenant> implements ISysTenantService {
    ...
    public void invitationUserJoin(String ids, String phone,String username) {
        ...
        if(oConvertUtils.isNotEmpty(phone)){
            ...
            //说明用户不存在
            if(null == userByPhone){
                throw new NeuronBootException("当前用户不存在，请核对手机号");
            }
        ...
        for (String id:idArray) {
            ...
            if(null == userTenant){
                ...
                throw new NeuronBootException(errMsg);
            }
        }
    }
        ...
        if(null == one){
            ...
            if(relation != null){
                ...
                if(CommonConstant.USER_TENANT_UNDER_REVIEW.equals(relation.getStatus())){
                    ...
                    throw new NeuronBootBizTipException("管理员已拒绝您加入租户,请联系租户管理员");
                }else if(CommonConstant.USER_TENANT_QUIT.equals(relation.getStatus())){
                    msg = ",状态:已离职";
                }
                throw new NeuronBootBizTipException("您已是该租户成员"+msg);
            }
            //用户加入门牌号审核中状态
   ...
   public int tenantIdGenerate(){
    ...
    public void exitUserTenant(String userId, String username, String tenantId) {
        ...
        if (username.equals(sysTenant.getCreateBy())) {
            ...
            if (null != userIdsByTenantId && userIdsByTenantId.size() > 1) {
                //需要指配拥有者
                throw new NeuronBootException("assignedOwen");
            } else if (null != userIdsByTenantId && userIdsByTenantId.size() == 1) {
                //只有拥有者的时候需要去注销租户
                throw new NeuronBootException("cancelTenant");
            } else {
                throw new NeuronBootException("退出租户失败，租户信息已不存在");
            }
        } else {
            //不是拥有者直接删除
    ...
    public void deleteUserByPassword(SysUser sysUser, Integer tenantId) {
        ...
        if(null == isHaveAdmin || 0 == isHaveAdmin){
            throw new NeuronBootException("您不是当前组织的管理员，无法删除用户！");
        }
        ...
        if(null == sysUserTenant || !CommonConstant.USER_TENANT_QUIT.equals(sysUserTenant.getStatus())){
            throw new NeuronBootException("用户没有离职，不允许删除！"); 
        }
    ...
    private void verifyCreateTimeAndPassword(SysUser sysUser,String password) {
        if(null == sysUser){
            throw new NeuronBootException("该用户不存在，无法删除！");
        }
        //step1 验证创建时间
        //当前登录用户
        LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        ...
        if(!sameDay){
            throw new NeuronBootException("用户不是今天创建的，无法删除！");
        }
        //step2 验证密码
        //获取admin的用户
        ...
        if(!passwordEncode.equals(adminUser.getPassword())){
            throw new NeuronBootException("您输入的密码不正确，无法删除该用户！");
        }
    }
    ...
    public void deleteUser(SysUser sysUser, Integer tenantId) {
        ...
        if(null == sysTenant || !user.getUsername().equals(sysTenant.getCreateBy())){
            throw new NeuronBootException("您不是当前组织的创建者，无法删除用户！");
        }
        ...
        if(CollectionUtils.isNotEmpty(sysUserTenants)){
            throw new NeuronBootException("该用户还存在于其它组织中，无法删除用户！");
        }
        //step3 验证创建时间和密码
        SysUser sysUserData = userService.getById(userId);
        this.verifyCreateTimeAndPassword(sysUserData,password);
