package com.echarge.modules.openapi.filter;
...
public class ApiAuthFilter implements Filter {
    ...
    protected void checkBlackList(OpenApi openApi, String ip) {
        ...
        if (blackList.contains(ip)) {
            throw new NeuronBootException("目标接口限制IP[" + ip + "]进行访问，IP已记录，请停止访问");
        }
    }
    ...
    protected void checkSignValid(String appkey, String signature, String timestamp) {
        if (!StringUtils.hasText(appkey)) {
            throw new NeuronBootException("appkey为空");
        }
        if (!StringUtils.hasText(signature)) {
            throw new NeuronBootException("signature为空");
        }
        if (!StringUtils.hasText(timestamp)) {
            throw new NeuronBootException("timastamp时间戳为空");
        }
        if (!timestamp.matches("[0-9]*")) {
            throw new NeuronBootException("timastamp时间戳不合法");
        }
        if (System.currentTimeMillis() - Long.parseLong(timestamp) > 5 * 60 * 1000) {
            throw new NeuronBootException("signature签名已过期(超过五分钟)");
        }
    }
    ...
    protected void checkSignature(String appKey, String signature, String timestamp, OpenApiAuth openApiAuth) {
        if(openApiAuth==null){
            throw new NeuronBootException("不存在认证信息");
        }

        if(!appKey.equals(openApiAuth.getAk())){
            throw new NeuronBootException("appkey错误");
        }

        if (!signature.equals(md5(appKey + openApiAuth.getSk() + timestamp))) {
            throw new NeuronBootException("signature签名错误");
        }
    }
    ...
    protected void checkPermission(OpenApi openApi, OpenApiAuth openApiAuth) {
        ...
        if (!hasPermission) {
            throw new NeuronBootException("该appKey未授权当前接口");
        }
    }
