package com.echarge.config.firewall.SqlInjection.impl;

import lombok.extern.slf4j.Slf4j;
import com.echarge.common.constant.SymbolConstant;
import com.echarge.common.exception.NeuronSqlInjectionException;
...
import com.echarge.config.firewall.interceptor.LowCodeModeInterceptor;
import com.echarge.modules.system.entity.SysTableWhiteList;
import com.echarge.modules.system.security.DictQueryBlackListHandler;
import com.echarge.modules.system.service.ISysTableWhiteListService;
...
public class DictTableWhiteListHandlerImpl implements IDictTableWhiteListHandler {
    ...
    private boolean isDev() {
        if (DictTableWhiteListHandlerImpl.LOW_CODE_IS_DEV == null) {
            if (this.neuronBaseConfig.getFirewall() != null) {
                String lowCodeMode = this.neuronBaseConfig.getFirewall().getLowCodeMode();
    ...
    private void throwException() throws NeuronSqlInjectionException {
        this.throwException(this.getErrorMsg());
    }

    private void throwException(String message) throws NeuronSqlInjectionException {
        if (oConvertUtils.isEmpty(message)) {
            message = this.getErrorMsg();
        }
        log.error(message);
        throw new NeuronSqlInjectionException(message);
    }
