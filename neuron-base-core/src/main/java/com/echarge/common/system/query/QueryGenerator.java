...
import com.echarge.common.system.util.SqlConcatUtil;
	...
	private static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper,Map<String, String[]> parameterMap, Map<String,String> fieldColumnMap) {
		...
		if (oConvertUtils.isNotEmpty(column) && oConvertUtils.isNotEmpty(order)) {
			...
			if (!allColumnExist(column, allFields)) {
				throw new NeuronBootException("请注意，将要排序的列字段不存在：" + column);
			}
...
//				|| "sort".equals(name) || "order".equals(name)
	...
	public static Map<String, SysPermissionDataRuleModel> getRuleMap() {
		...
		try {
			list = NeuronDataAutorUtils.loadDataSearchConditon();
		}catch (Exception e){
