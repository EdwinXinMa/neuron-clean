package com.echarge.config.firewall.interceptor;
...
public class LowCodeModeInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        //1、验证是否开启低代码开发模式控制
        if (neuronBaseConfig == null) {
            neuronBaseConfig = SpringContextUtils.getBean(NeuronBaseConfig.class);
        }
        if (commonAPI == null) {
            commonAPI = SpringContextUtils.getBean(CommonAPI.class);
        }

        if (neuronBaseConfig.getFirewall()!=null && LowCodeModeInterceptor.LOW_CODE_MODE_PROD.equals(neuronBaseConfig.getFirewall().getLowCodeMode())) {
