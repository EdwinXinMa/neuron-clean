...
public class SqlInjectionUtil {
	...
	public static void filterContent(String value, String customXssString) {
		...
		for (int i = 0; i < xssArr.length; i++) {
			if (value.indexOf(xssArr[i]) > -1) {
				...
				throw new NeuronSqlInjectionException(SqlInjectionUtil.SQL_INJECTION_TIP + value);
			}
		}
	...
	public static void checkSqlAnnotation(String str){
		...
		if(matcher.find()){
			...
			log.error(error);
			throw new NeuronSqlInjectionException(error);
		}
	}
	...
	public static String getSqlInjectTableName(String table) {
		...
		if (!isValidTableName) {
			...
			log.error(errorMsg);
			throw new NeuronSqlInjectionException(errorMsg);
		}
